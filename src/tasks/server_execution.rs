...
pub async fn server_startup(database_pool: sqlx::PgPool, upsert_channel: Sender<()>) -> Result<(), ServerStartupError> {
    ...
    upsert_channel.send(()).map_err(|_| ServerStartupError).into_report()
        .change_context(ServerStartupError)
