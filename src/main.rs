...
async fn run_server() -> Result<(), ServerError> {
    ...
    let database_url = std::env::var("DATABASE_URL").into_report()
        .change_context(ServerError)
        .attach("Failed to get environment variable `DATABASE_URL`, needed to connect to database.")?;
    let api_key = std::env::var("ALPHA_VANTAGE_API_KEY").into_report()
        .change_context(ServerError)
    ...
    let upsert_task = tokio::spawn(tasks::recurring_get_raw_data(pool.clone(), task_recv, api_key, 1));
    log::trace!("Starting up server.");
    let server_task = tokio::spawn(tasks::server_startup(pool.clone(), task_send));

    let (upsert_res, server_res) = tokio::join!(
        upsert_task,
        server_task
    );
